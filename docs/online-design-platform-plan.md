...
| template_requirements | jsonb | 风格、模板、skill、约束 |
...
模板和 skill 由 Application Service 负责授权和版本快照，由 Runtime Gateway 编译为受控 prompt block。用户前端不得直接拼接 BabeL-O 私有 prompt 结构。
...
### 7.2.2 Capability Distribution System

DUDesign 需要提供模板、插件和自动化 loop 组成的能力分发系统。该能力单独维护在 `docs/modules/capability-distribution/`。

- Templates：面向用户收敛为“场景 / 视觉 / 高级”。底层仍用 `DomainTemplate / AestheticProfile / ColorPalette` 等结构化契约。
- Plugins：集成声明式 skill、MCP tool binding、权限策略和工具 scope。
- Automation Loop：定义生成、调试、验证、修复、再验证和停止条件。
- 用户偏好按 user/workspace scope 隔离，并可作为生成提示，但不能替代用户显式选择。
- 所有能力配置必须版本化，job 创建时保存所用版本快照，保证旧 session resume 不被新版本影响。
- Runtime Gateway 负责将 template/plugin/loop 编译为安全 prompt context、tool policy 和 loop plan，禁止覆盖 runtime cwd、工具权限和文件路径安全约束。
...
- 色板：作为视觉风格的可选覆盖项。
- 补充风格词：用户临时 prompt modifier，不进入官方 registry。
- 参考品牌：只允许 inspiration-only 抽象参考，不复制品牌资产、商标或专有文案。
- 负面要求：例如不要深色背景、不要卡片堆叠、不要大面积渐变。

Open Design 参考启发：

- 不把所有生成能力都叫“模板”。
- 将输出形态、使用场景、设计系统、skill/template 和 prompt gallery 分层治理。
- DUDesign MVP 先保留 HTML 页面生成主线，前端用单个“设计方向”选择器承载场景、视觉和高级约束。
- 中期引入 Design System 能力，用于沉淀品牌 token、排版、组件、动效、语气和反模式，可从已有 HTML、用户素材或历史 variation 中提取。
- Prompt starter / 灵感模板可作为单独 gallery，默认只填充 brief，不直接进入正式 capability snapshot。

`DESIGN.md` 生态参考启发：

- `google-labs-code/design.md` 更适合作为规范与校验参考：YAML front matter 存机器可读 token，Markdown 正文存设计意图和规则。
- `awesome-design-md` 更适合作为模板库组织方式参考：按行业、产品类型和视觉气质整理可复用设计系统。
- DUDesign 应支持导入/导出 `DESIGN.md`，但内部保留 `DesignTemplatePack` 或等价 stable contract，避免外部 alpha 规范变化影响业务核心。
- 官方模板应做成通用启发式模板，例如 Premium Product Gallery、Dark Developer Tool、Minimal SaaS、Editorial Landing、Fintech Trust、Creative Portfolio，不直接复制公开品牌名称、logo、专有字体、商标元素或明显 trade dress。
- 创建 job 时将 template + skill + loop + 用户偏好组合成 `CapabilityProfile` snapshot；每个 variation 可以绑定不同 template pack，以支持同一 prompt 下生成真正不同的视觉方向。
...
- 结果墙。
- 官方模板选择。
- template/skill prompt context 注入。
- 设计方向选择器：场景、视觉、高级。

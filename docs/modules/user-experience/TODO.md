...
- [ ] 在首页 composer 增加产品模式切换：`Web&App / 动态百科卡片`。
- [ ] 保留“新建 HTML / 基于已有 HTML”输入来源语义，不用产品模式替换 source mode。
- [ ] 动态百科卡片模式下，将主输入文案切换为词条名称/词条内容输入。
- [ ] 切换到动态百科卡片模式后自动勾选：
  - [ ] 词条引导 skill。
  - [ ] 动态百科词条卡片模板包。
  - [ ] 自动审查 loop。
- [ ] 自动勾选结果必须可见、可撤销，并写入最终 job snapshot。
- [ ] 展示词条引导返回的分类结果、置信度、推荐子模板和推荐理由。
- [ ] 低置信度时支持用户确认或改选分类/子模板后再生成。
- [ ] 半自动审查模式下展示审查报告，并支持确认修复、跳过、手动修改。
- [ ] E2E：切换动态百科卡片模式 -> 自动勾选三件套 -> 输入词条 -> 确认推荐 -> 创建 job。

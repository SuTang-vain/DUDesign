...
export async function createVariationThroughUi(page: Page, prompt: string): Promise<void> {
  ...
  await expect(page.getByTestId('variation-code-stream').first()).toContainText('<!doctype html>')
  await expect(page.getByTestId('variation-code-stream').first()).toContainText(/lines ·/)
  await page.getByTestId('variation-code-stream').first().getByTestId('copy-code-button').click()
  await expect(page.getByTestId('variation-code-stream').first().getByTestId('copy-code-button')).toContainText(/Copied|Copy failed/)
  await page.getByRole('button', { name: 'styles.css' }).first().click()
  await expect(page.getByTestId('variation-code-stream').first()).toContainText('.hero')

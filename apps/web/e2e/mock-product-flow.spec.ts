...
test('composer menus close on outside click and do not stack', async ({ page }) => {
  ...
  await expect(page.locator('.paired-child-panel')).toBeHidden()
  await page.getByTestId('template-panel-options').getByRole('button', { name: 'Domain' }).click()
  await expect(page.locator('.paired-child-panel')).toBeVisible()
...
test('workbench can choose capability distribution options', async ({ page }) => {
  ...
  await page.getByTestId('template-panel-options').getByRole('button', { name: 'Aesthetic' }).click()
  await page.getByTestId('aesthetic-profile-options').getByRole('button', { name: /Apple-like Minimal/ }).click()
  await page.getByTestId('template-panel-options').getByRole('button', { name: 'Palette' }).click()
  ...
  await expect(page.getByTestId('capability-summary')).toContainText('Apple-like Minimal')
  ...
  await expect(page).toHaveURL(/\/jobs\/job_/)
  await expect(page.getByTestId('variation-grid')).toBeVisible()
  await expect(page.getByTestId('job-capability-snapshot')).toContainText('Apple-like Product Page')
  await expect(page.getByTestId('job-capability-snapshot')).toContainText('Apple-like Minimal')
  ...
  await expect(page.getByTestId('variation-capability-snapshot')).toContainText('Apple-like Product Page')
  await expect(page.getByTestId('variation-capability-snapshot')).toContainText('Apple-like Minimal')

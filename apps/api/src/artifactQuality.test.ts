...
<html>
  ...
  <body>
    ...
    assert.equal(report.status, 'fail')
    assert.ok(report.issues.some(issue => /Desktop democase-derived first view exposes 13 visible controls/i.test(issue)))
    assert.ok(report.issues.some(issue => /separate control groups/i.test(issue)))
  })

  it('applies the matched democase stage budget instead of only the global control ceiling', async () => {
  ...
  <head>
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      body { display: grid; place-items: center; background: #eef1f8; font-family: sans-serif; }
      main { width: 788px; height: 492px; padding: 28px; overflow: hidden; background: #fff; }
      nav { display: flex; gap: 6px; margin: 24px 0; }
      button { min-width: 64px; height: 36px; }
      [hidden] { display: none !important; }
      @media (max-width: 320px), (max-height: 365px) {
        main { width: 300px; height: 360px; padding: 16px; }
        nav { gap: 4px; margin: 14px 0; }
        button { min-width: 0; flex: 1; height: 32px; }
        button:nth-child(n + 4) { display: none; }
      }
    </style>
  </head>
  <body>
    <main data-dudesign-template="timeline-card">
      <h1>人物事件链</h1>
      <p>首屏应围绕一个活动阶段展开。</p>
      <nav role="tablist">
        ${Array.from({ length: 9 }, (_, index) => `<button role="tab" aria-selected="${index === 0}" data-phase="${index + 1}">阶段${index + 1}</button>`).join('')}
      </nav>
      <section id="phase-detail" role="tabpanel" aria-live="polite">当前阶段 1 的核心事实。</section>
      <section hidden>其余阶段的原因、结果和来源说明。</section>
    </main>
    <script>
      document.querySelectorAll('[role="tab"]').forEach((tab) => tab.addEventListener('click', () => {
        document.querySelectorAll('[role="tab"]').forEach((item) => item.setAttribute('aria-selected', 'false'));
        tab.setAttribute('aria-selected', 'true');
        document.querySelector('#phase-detail').textContent = '当前阶段 ' + tab.dataset.phase + ' 的核心事实。';
      }));
    </script>
  </body>
</html>`
    const baseline = await analyzeHtmlArtifactQualityWithPixelGate(html, { enabled: true })
    const profiled = await analyzeHtmlArtifactQualityWithPixelGate(html, {
      enabled: true,
      experienceProfile: {
        dominantStage: 'timeline_story',
        firstViewPromise: 'Show one active phase.',
        primaryInteraction: 'Switch the active phase.',
        secondaryReveal: 'Reveal later milestones locally.',
        attentionBudget: {
        ...
        preserveAt300x360: ['topic title', 'active phase', 'phase switcher'],
        deferAt300x360: ['inactive phases'],
        forbiddenPatterns: ['all milestones expanded at once'],
      },
    })

    assert.equal(baseline.issues.some(issue => /exposes 9 visible controls/i.test(issue)), false)
    assert.equal(profiled.status, 'fail')
    assert.ok(profiled.issues.some(issue => /for timeline_story exposes 9 visible controls; keep at most 8/i.test(issue)))
  })

  it('fails an extreme-small first view that exceeds the matched visible-item budget', async () => {
    const report = await analyzeHtmlArtifactQualityWithPixelGate(`<!doctype html>
<html>
  <head>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <style>
      * { box-sizing: border-box; }
      ...
      button { min-width: 88px; height: 34px; }
      .items { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; }
      article { min-height: 54px; padding: 10px; background: #f4f5f7; }
      [hidden] { display: none !important; }
      @media (max-width: 320px), (max-height: 365px) {
        main { width: 300px; height: 360px; padding: 14px; }
        nav { margin: 8px 0; }
        button { flex: 1; min-width: 0; height: 32px; }
        .items { grid-template-columns: repeat(2, 1fr); gap: 6px; }
        article { min-height: 54px; padding: 8px; }
      }
    </style>
  </head>
  <body>
    <main data-dudesign-template="relation-card">
      <h1>主题关系探索</h1>
      <p>首屏保留一个核心关系说明。</p>
      <nav role="tablist">
        <button role="tab" aria-selected="true" aria-controls="primary">核心</button>
        <button role="tab" aria-selected="false" aria-controls="more">更多</button>
      </nav>
      <section id="primary" role="tabpanel" class="items">
        <article>关系甲</article><article>关系乙</article><article>关系丙</article><article>关系丁</article>
      </section>
      <section id="more" role="tabpanel" hidden>点击后揭示的次级关系。</section>
    </main>
    <script>
      document.querySelectorAll('[role="tab"]').forEach((tab) => tab.addEventListener('click', () => {
        document.querySelectorAll('[role="tab"]').forEach((item) => item.setAttribute('aria-selected', String(item === tab)));
      ...
      experienceProfile: {
        ...
        firstViewPromise: 'Show one selected relation.',
        primaryInteraction: 'Select a relation.',
        secondaryReveal: 'Reveal additional relations locally.',
        attentionBudget: {
          desktop: { maxControlGroups: 2, maxVisibleControls: 8, maxVisibleItems: 6 },
          extremeSmall: { maxControlGroups: 2, maxVisibleControls: 5, maxPrimaryTabs: 3, maxVisibleItems: 3, maxTextCharacters: 420 },
        },
        preserveAt300x360: ['topic title', 'selected relation', 'relation switcher'],
        deferAt300x360: ['additional relations'],
        forbiddenPatterns: ['all relations expanded at once'],
      },
    })

    assert.equal(report.status, 'fail')
    assert.ok(report.issues.some(issue => /for relation_map renders 4 equal-level visible items; keep at most 3/i.test(issue)))
  })
})

...
describe('Design job event persistence and partial failures', () => {
  ...
  it('replays persisted job events through SSE after the producing event bus is gone', async () => {
    ...
    try {
      ...
      assert.equal(producedSnapshot.variations[0]?.execution.phase, 'completed')
      assert.equal(producedSnapshot.variations[0]?.execution.attempt, 1)
      assert.equal('runtimeLaneId' in (producedSnapshot.variations[0] ?? {}), false)
      const persistedVariation = await store.getVariationById(producedSnapshot.variations[0]!.id)
      assert.equal(persistedVariation?.runtimeLaneId, 'lane-controlled-1')
      assert.equal(persistedVariation?.runtimeBackendId, 'backend-controlled-1')
      assert.equal(persistedVariation?.runtimeLeaseId, 'lease-controlled-1')
      assert.equal(persistedVariation?.runtimeAttempt, 1)

...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse, service: ApplicationService): Promise<void> {
  ...
  const variationReviewActionMatch = url.pathname.match(/^\/api\/variations\/([^/]+)\/review-actions$/)
  if (method === 'POST' && variationReviewActionMatch) {
    sendJson(res, 200, await service.reviewVariationAction(
      ctx,
      decodeURIComponent(variationReviewActionMatch[1]!),
      await readJson(req),
    ))
    return
  }
